...
class Compras {
  ...
  int id_compra_detalle = 0,idcompra = 0,idproducto = 0;
  ...
  int leeridProveedores() {
    ...
    cn.abrir_conexion();
    if (cn.getConectar()) {
      string consulta = "SELECT idProveedore,proveedor FROM Proveedores  WHERE nit = '" + nit + "'";
  ...
  void eliminar() {
    ...
    if (cn.getConectar()) {
      string deleteQueryDetalle = "DELETE FROM compras_detalle WHERE idcompra = '" + to_string(idcompra) + "'";
      ...
      if (!q_estado) {
        ...
        cout << "Ocurri� un problema al eliminar los registros de la tabla compras_detalle: " << mysql_error(cn.getConectar()) << endl;
        cn.cerrar_conexion();
        return;
      }
      ...
      if (!q_estado) {
        ...
        cout << "Ocurri� un problema al eliminar los registros de la tabla compras: " << mysql_error(cn.getConectar()) << endl;
      }
    } else {
      cout << "Error al conectar" << endl;
    }
